...
@Config
@TeleOp(name = "Artifact Tracking", group = "Vision")
public class Artifact_Tracking extends LinearOpMode {
        ...
        while (opModeIsActive()) {
            ...
            if (result.isValid()) {
                ...
                for (LLResultTypes.ColorResult cr : colorResults) {
                    tYaw = cr.getTargetPoseRobotSpace().getOrientation().getYaw();
                    z_pose = cr.getTargetPoseRobotSpace().getPosition().z;
                    tx = cr.getTargetXDegrees();
                    ty = cr.getTargetYDegrees();
                    ty = cr.getTargetYDegrees();
                }
...
//                    yaw = fr.getTargetPoseRobotSpace().getOrientation().getYaw();
            ...
            if (gamepad1.left_bumper && !targetFound){
                ...
                Find_Artifact(telemetry);

            }
            else if (gamepad1.left_bumper && targetFound) {

                Track_Artifact(telemetry);
    ...
    public static void Track_Artifact(Telemetry telemetry){
    ...
    public static void Find_Artifact(Telemetry telemetry){
